<div>
    @push('styles')
        ...
        <style>
            ...
            /* nice red focus ring for native elements that don't get Tailwind classes */
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                >
                    <div>
                        ...
                        <div class="mt-1 flex gap-2">
                            ...
                            <button type="button"
                                    ...
                                    @click="search()">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="size-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-search-icon lucide-search"><path d="m21 21-4.34-4.34"/><circle cx="11" cy="11" r="8"/></svg>
                                Search
                            </button>
                    ...
                    <div x-ref="mapEl" id="address-map"
                         class="h-80 w-full rounded-xl overflow-hidden ring-1 ring-slate-200/70"
                         wire:ignore></div>

                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div class="md:col-span-2">
                            <label class="text-sm font-medium">Delivery Address (autofilled)</label>
                            <input type="text" class="w-full focus:outline-none focus:border-none focus:ring-2 focus:ring-customRed-100 rounded"
                                   :disabled="!manualOverride"
                                   :class="manualOverride ? '' : 'opacity-80 cursor-not-allowed'"
                                   x-model="addressText"
                                   @input="$wire.set('address', addressText)" />
                            ...
                            <label class="inline-flex items-center gap-2 text-sm mt-2 select-none">
                                <input id="manual_override" type="checkbox" class="checkbox checkbox-sm border-red-300 text-red-500 focus-red"
                                       x-model="manualOverride"
                                       @change="!manualOverride && (addressText = savedAddressText); $wire.set('address', addressText)" />
                        ...
                        <div>
                            ...
                            <input type="text" class="focus:outline-none focus:border-none focus:ring-2 focus:ring-customRed-100 rounded w-full"
                                   x-model="city" @input="$wire.set('city', city)" placeholder="City">
                        </div>
                        <div>
                            <label class="text-sm font-medium">Postcode (auto)</label>
                            <input type="text" class="focus:outline-none focus:border-none focus:ring-2 focus:ring-customRed-100 rounded w-full"
                                   x-model="postcode" @input="$wire.set('postcode', postcode)" placeholder="e.g. 1212">
                        </div>

                        <div class="md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <label class="text-sm font-medium">Contact name (optional)</label>
                                <input type="text" class="focus:outline-none focus:border-none focus:ring-2 focus:ring-customRed-100 rounded w-full"
                                       wire:model.defer="contact_name" />
                            </div>
                            <div>
                                <label class="text-sm font-medium">Contact phone (optional)</label>
                                <input type="text" class="focus:outline-none focus:border-none focus:ring-2 focus:ring-customRed-100 rounded w-full"
                ...
                <div class="md:col-span-2" x-data="{ note: @entangle('customer_note').defer }">
                    ...
                    <textarea class="focus:outline-none focus:border-none focus:ring-2 focus:ring-customRed-100 rounded w-full" rows="3"
                              maxlength="200" x-model="note"></textarea>
                    @error('customer_note') <p class="text-red-600 text-xs mt-1">{{ $message }}</p> @enderror
                </div>
            </div>
                ...
                return {
                    ...
                    locateNow(btnEl) {
                        ...
                        const ok = (pos) => {
                            ...
                            this.locating = false; btnEl?.removeAttribute('disabled');
                        };
                        const fail = (err) => {
                            this._overlay(false); this.locating = false; btnEl?.removeAttribute('disabled');
                    ...
                    async search() {
                        ...
                        const res = await fetch(url, {
                            headers: { 'Accept': 'application/json', 'User-Agent': 'CloudBite/1.0 (contact@example.com)' }
                        });
                    ...
                    async _updatePosition(lat, lng, reverse = false) {
                        ...
                        try {
                            ...
                            this.$wire.set('address', this.addressText);
                            this.$wire.set('city', this.city);
                            this.$wire.set('postcode', this.postcode);
                        } catch (_) {}
                    },
                    ...
                    _pushNullsToWire() {
                        this.$wire.set('lat', null);
                        this.$wire.set('lng', null);
                        this.$wire.set('address', '');
                        this.$wire.set('city', '');
                        this.$wire.set('postcode', '');
