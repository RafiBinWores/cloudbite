    @push('meta')
        <title>{{ $seoTitle ?? 'Dishes | CloudBite' }}</title>

        <meta name="description"
            content="{{ $seoDesc ?? 'Browse our full menu—fresh meals, snacks, and chef-crafted dishes delivered fast across Dhaka.' }}">
        ...
        <meta property="og:title" content="{{ $seoTitle ?? 'Dishes | CloudBite' }}">
        ...
        <meta name="twitter:title" content="{{ $seoTitle ?? 'Dishes | CloudBite' }}">
        <meta name="twitter:description"
            content="{{ $seoDesc ?? 'Browse our full menu—fresh meals, snacks, and chef-crafted dishes delivered fast across Dhaka.' }}">
        <meta name="twitter:image" content="{{ $seoImage ?? asset('assets/images/seo/default-og.jpg') }}">
    @endpush

    <div x-data="{ filtersOpen: false }" x-init="$watch('filtersOpen', value => {
        if (value) {
                ...
                <section class="col-span-12 lg:col-span-9 lg:ps-2">
                    <div class="flex flex-wrap items-center justify-between gap-3 font-jost">
                        ...
                        <p class="text-base sm:text-lg hidden lg:block">
                            @if ($dishes->total())
                                Showing {{ $dishes->firstItem() }}–{{ $dishes->lastItem() }} of
                                {{ $dishes->total() }}
                                results
                            @else
                                No results found
                    ...
                    <!-- Cards Grid -->
                    <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-3 gap-6 mt-6">
                                    ...
                                    <figure class="relative">
                                        ...
                                        @if ($dish->discount && $dish->discount_type)
                                            <span
                                                ...
                                                @if ($dish->discount_type === 'percent')
                                                    {{ $discountValue }} <span class="ps-1 font-jost">&#x25;
                                                        OFF</span>
                                                @elseif($dish->discount_type === 'amount')
                                                    {{ $discountValue }}
                                    ...
                                    <div class="card-body p-3">
                                        ...
                                        <div class="flex items-center justify-between mt-2 gap-2 md:items-center">
                                            <div class="font-oswald text-customRed-100">
                                                @if ($hasVariations)
                                                    <div
                                                        class="leading-tight md:leading-normal flex items-center gap-1">
